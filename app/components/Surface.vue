<template>
  ...
  <div
    ...
    ref="glassRef"
    :class="['surface-glass', span ? `surface-glass--${span}` : '', directionClass]"
    :style="surfaceStyles"
  >
    <slot />
  </div>
...
.is-row { flex-direction: row !important; }
.is-column { flex-direction: column !important; }
...
.surface-glass:hover::before {
    opacity: 1.0; /* kept full — hover is subtle via box-shadow brightening */
}
...
.surface-glass::after {
    ...
    inset: 0;
    border-radius: inherit;
    padding: 2px;          /* slightly wider than the 1.5px border */
    box-sizing: border-box;
    background: radial-gradient(
        circle 240px at var(--mouse-x, -999%) var(--mouse-y, -999%),
...
@media (hover: none) {
    .surface-glass::after { display: none; }
}
